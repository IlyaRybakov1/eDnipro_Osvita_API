...
const fetchKindergartensBasic = async (
  ...
  try {
    ...
    return response.data.elements.map((school: any) => ({
      ...
      name: school.tags.name || "Unnamed Kindergarten",
      location: {
    ...
    throw new Error("Failed to fetch basic kindergarten data");
  }
};

const fetchKindergartenDetailsById = async (
  id: string
  ...
  try {
    ...
    throw new Error(`Failed to fetch kindergarten details with id ${id}`);
  }
};

const fetchRandomKindergartens = async (
  numSchools: number = 3
): Promise<SchoolDetails[]> => {
  try {
    const schools = await fetchKindergartensBasic();
    ...
    throw new Error("Failed to fetch random kindergarten data");
  }
};

export default {
  fetchKindergartensBasic,
